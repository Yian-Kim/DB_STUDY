...
- 가장 중요한 작업
- 설계도 작업(***)
- 요구분석 + 데이터베이스를 구성하기 위해 필요한 정보(Raw 데이터) 수집 > 분석 > 도식화(설계도)
...
4. Attribute, 속성(특성)
- 엔티티가 가지는 특성(정보)
- '사원' 엔티티 = 사원명 + 사원번호 + 연락처 등등 <- 속성
- 컬럼(=셀 값, 데이터), 멤버 변수

5. Tuple, 튜플
- 엔티티에 정의된 규칙으로 만들어진 실제 데이터
- '사원' 엔티티 = 사원명 + 사원번호 + 연락처 등등
...
- 복합키, Composite key
- 레코드(행)을 구분할 수 있는 키
- 1개의 키만 가지고는 식별자가 불가능한 키들 > 2(1)개의 조합으로 기본키 역할
...
정규화, Normaliztion
- 자료의 손실이나 불필요한 정보를 없애고, 데이터의 일관성을 유지하고, 데이터의 중복을 
    최소화하기 위해 자료 구조(테이블 스키마)를 수정하는 작업
...
- 동일한 데이터가 2개 이상의 테이블에 존재 > 그 중 1개를 수정했지만 다른 테이블의 데이터는
    수정하지 못했을 경우 > 두 테이블의 데이터가 서로 달라지는 상황
    

함수 종속(Functional Dependency)
- 1개의 테이블내에서 2개 이상의 속성(컬럼)들끼리 관계 제약
...
- 이 규칙을 만족하도로록 테이블 수정 > 정규화한다.

- 모든 컬럼(속성)은 원자값을 가진다.(**************)
    - 여러개의 값들을 하나의 컬럼값에 넣으면 안된다.
...
- 개설 과정 관리
    - 관리자는 여러 개의 개설 과정을 등록 및 관리할 수 있어야 한다.
...
메인 > 관리자 > 개설 과정 관리
메인 > 관리자 > 개설 과정 관리 > 개설 과정 등록 -- AB-001
    - 과정명 입력
    - 과정기간(시작일) 입력
    - 과정기간(종료일) 입력
    - 강의실 정보 출력(상태 출력) > 강의실 정보 선택 > 입력

강의실 정보 출력(상태 출력) -- AB-003

메인 > 관리자 > 개설 과정 관리 > 개설 과정 목록보기 --AB-002
    - 개설 과정 목록 출력(표)
    
메인 > 관리자 > 개설 과정 관리 > 개설 과정 수정
메인 > 관리자 > 개설 과정 관리 > 개설 과정 삭제
...
(
    ...
    begin varchar2(15) not null,
    end varchar2(15) not null,
    classroomseq number
        references Classromm(seq) not null
);
...
-- 더미 데이터
insert into Classroom values (classroom_seq.nextval, '1강의실', 30);
insert into Classroom values (classroom_seq.nextval, '2강의실', 30);
insert into Classroom values (classroom_seq.nextval, '3강의실', 26);
--...
-- 개설 과정
insert into Course values (course_seq.nextval, '자바 프로그래머 과정', 
                                '2019-01-02', '2019-07-25', 1);
...
--강의실 정보 출력(상태 출력) -- AB-003
select seq, name, capacity, ing from Classroom order by ..


--메인 > 관리자 > 개설 과정 관리 > 개설 과정 목록보기 --AB-002
select 컬럼리스트.. from Course order by..
